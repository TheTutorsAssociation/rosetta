...
A few things are **not obvious** — they're called out below.

## Topology

```
browser ──▶ frontend (vite dev :5001, or the SSR node server in prod)
                  │
                  ├─ browser fetch ─┐
                  └─ SSR loader/action fetch ─┐
                                              ▼
                              backend  (uvicorn :5000)
...
`http://localhost:5000` (the backend dev server). The backend's `allowed_origins` (in
`backend/app/core/config.py`) already includes the frontend dev origin `http://localhost:5001`. Both
ports are overridable via `PORT`.
...
3. Backend `auth_user` decodes/validates the JWT (which carries the user's id and `user_type` claim) and
   sets `request.state.user`; protected routes use `Permission.*` dependencies.
...
The browser enforces CORS on cross-origin requests (frontend `:5001` → backend `:5000`). Keep the
frontend's origin in the backend's `allowed_origins`:

- **Dev:** `http://localhost:5001` is already in the default `allowed_origins`.
...
| Base URL | `base_url` = `:5000` | `VITE_API_BASE_URL` = `:5000` | must match |

Nothing **enforces** this contract across repos — if you change the error key or a field name on one
side, update the other (and its tests). Consider generating the frontend types from the backend's
OpenAPI schema (`/openapi.json`) if you want it enforced. When you add list endpoints, follow the
`tc-fullstack-starter` template's `PaginatedResponse` shape so both sides stay aligned.

## Local full-stack dev

```bash
# terminal 1 — backend (needs Postgres + Redis running locally)
cd backend && make install-dev && uv run alembic upgrade head && make run-dev   # :5000

# terminal 2 — backend worker (only if you use Celery tasks)
cd backend && make run-worker

# terminal 3 — frontend
cd frontend && npm ci && cp .env.example .env && npm run dev                      # :5001 → :5000
```

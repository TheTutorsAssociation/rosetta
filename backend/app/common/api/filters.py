...
class ListOrder:
    ...
    produce a deterministic order within rows that share the primary sort value. A deterministic
    ``model.id`` tiebreaker is always appended so pagination is stable even when the primary sort
...
class ListFilter(BaseModel):
    ...
    @classmethod
    def get_options(cls) -> dict:
        """Return filter metadata (field type + enum choices) for OPTIONS responses."""
        data = {}
